...
#   scripts/install.ps1  sha256=aedfaf9ac1a54d2ae992115fe98c72c218e479b5bd140b84f7e5457e9d4e4000
...
#--- Shared helpers ---------------------------------------------------------
# Output helpers, elevation check, port probing, prompts, secret generation and
# the Caddyfile generator are shared with scripts/install-native.ps1 (ADR-030's
# second delivery path) and live in scripts/lib/common.ps1. The Caddyfile output
# for every Docker case was verified byte-identical to the previous inline
# version before this rewire.
...
function Resolve-Autostart {
    ...
    Write-Host "    at boot, again at sign-in, and retries every 10 minutes."
    Write-Host ""
    Write-Host "    Be aware of the limit, because it decides whether a reboot is safe:" -ForegroundColor Yellow
    Write-Host "    the boot attempt runs in session 0, and Docker Desktop's WSL2 backend"
    Write-Host "    CANNOT start there -- no password and no retry changes that. On this"
    Write-Host "    host the gateway therefore comes up when an administrator signs in,"
    Write-Host "    not at boot. For genuinely unattended reboots, move the gateway to a"
    Write-Host "    Linux runtime (Docker Engine + systemd) -- a Hyper-V VM on this host or"
    Write-Host "    a separate Linux host, see ADR-029 -- or enable automatic sign-in for"
    Write-Host "    the account that owns Docker Desktop. A native Windows docker service"
    Write-Host "    (Docker CE / Mirantis) is NOT a fix: it runs Windows containers only."
    Write-Host ""
    Write-Host "    You'll be asked for the current user's password (stored by Windows so"
    Write-Host "    the task can run without an interactive prompt)."
...
function Install-Autostart {
    ...
    try {
        ...
        Write-Ok "Autostart task registered (boot + sign-in triggers, 10-minute retry)."
        Write-Substep "On a Docker Desktop host the stack comes up when an admin signs in, not at boot."
    } catch {
...
function Install-CertWatchdog {
    ...
    try {
        ...
        Invoke-WebRequest -Uri "$PUBLIC_REPO_BASE/scripts/host/setup-cert-watchdog.ps1" `
            -OutFile $script -UseBasicParsing -ErrorAction Stop
        & $script -InstallDir $Script:EffInstallDir
        Write-Ok "Certificate watchdog registered (checks every 1h + 5 min after each boot)."
    } catch {
...
function New-PostgresPassword {
    ...
    # 32 hex chars (128 bits of entropy). New-RandomHex (lib\common.ps1) uses
    # RandomNumberGenerator, not Get-Random, which is not cryptographic.
    $Script:EffPostgresPassword = New-RandomHex -Bytes 16
...
function New-ConfigProtectorSeed {
    ...
    $existingSeed = Get-EnvFileValue -Path $envPath -Key 'CONFIG_PROTECTOR_SEED'
    if ($existingSeed) {
        ...
        $Script:EffConfigProtectorSeed = New-RandomHex -Bytes 32
...
"@
    ...
    # The Caddyfile itself is generated by lib\common.ps1 so the native
    # installer (ADR-030) emits the identical, hard-won configuration -- the
    # auto_https/CA-download subtleties below the surface are not worth having
    # two copies of. Verified byte-identical to the previous inline version for
    # every Docker case (auto/letsencrypt x http port present/absent).
    $caddyfile = New-CaddyfileText `
        -HostName $Script:EffHostname `
        -Port     $Script:EffPort `
        -HttpPort $Script:EffHttpPort `
        -Tls      $Script:EffTls `
        -Upstream 'app:5000' `
        -CaddyDataRoot '/data/caddy'
